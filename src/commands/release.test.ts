...
beforeEach(async () => {
  testDir = await mkdtemp(join(tmpdir(), 'sc-release-test-'));
...
describe('sc release - validation errors', () => {
...
describe('sc release - safety checks', () => {
  ...
  test('succeeds when clean worktree with all commits pushed', async () => {
    ...
    expect(result).toBe(0);
    expect(logMessages.join(' ')).toContain('Released');
    expect(logMessages.join(' ')).toContain('ready');
  });
});
