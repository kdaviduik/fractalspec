...
describe('sc done - force threading', () => {
  test('passes force=true to completeSpec when --force is used', async () => {
    const spec = makeSpec('a1b2');
    await writeSpec(spec);
    ...
    spyOn(claimLogic, 'isSpecClaimed').mockResolvedValue(true);
    spyOn(claimLogic, 'checkClaimSafety').mockResolvedValue(mockSafetyResult);
    const completeSpy = spyOn(claimLogic, 'completeSpec').mockResolvedValue();

    await command.execute(['a1b2', '--force']);
    expect(completeSpy).toHaveBeenCalledWith(expect.objectContaining({ id: 'a1b2' }), true);
  });

  test('passes force=false to completeSpec when --force is not used', async () => {
    const spec = makeSpec('a1b2');
    await writeSpec(spec);
    ...
    expect(completeSpy).toHaveBeenCalledWith(expect.objectContaining({ id: 'a1b2' }), false);
  });
});
